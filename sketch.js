  ...
  if (displayGoing) {
    ...
    if (imgLoaded && images.length > 0) {
      ...
      if (poses.length > 0) {
        ...
        let upperArmSize =
          dist(leftShoulder.x, leftShoulder.y, leftElbow.x, leftElbow.y) / 2;
        let lowerArmSize =
          dist(leftElbow.x, leftElbow.y, leftWrist.x, leftWrist.y) / 2;
        let upperLegSize =
          dist(leftHip.x, leftHip.y, leftKnee.x, leftKnee.y) / 2;
        let lowerLegSize =
          dist(leftKnee.x, leftKnee.y, leftAnkle.x, leftAnkle.y) / 2;

        let wordsT = createGraphics(
          torsoD * 2,
          dist(
            leftShoulder.x,
            leftShoulder.y,
        ...
        push();
        translate(
          (leftShoulder.x + rightShoulder.x) / 2 - width / 2,
          (leftShoulder.y + rightShoulder.y) / 2 +
            (leftHip.y + rightHip.y) / 2 / 2.8 -
            height / 2,
          0
        );
        rotateY(radians(40));
        ...
        let wordsUA = createGraphics(
          upperArmSize * 2,
          dist(leftShoulder.x, leftShoulder.y, leftElbow.x, leftElbow.y) / 2
        );
        wordsUA.scale(-0.7, 0.7);
        wordsUA.translate(-wordsUA.width, 0);
        wordsUA.fill(255);
        wordsUA.textAlign(CENTER);
        wordsUA.text(displayedText, 0, 20);

        let wordsLA = createGraphics(
          lowerArmSize * 2,
          dist(leftElbow.x, leftElbow.y, leftWrist.x, leftWrist.y) / 2
        );
        wordsLA.scale(-0.7, 0.7);
        ...
        cylinder(
          upperArmSize,
          dist(leftShoulder.x, leftShoulder.y, leftElbow.x, leftElbow.y)
        );
        pop();
        ...
        cylinder(
          lowerArmSize,
          dist(leftElbow.x, leftElbow.y, leftWrist.x, leftWrist.y)
        );
        pop();
        ...
        cylinder(
          upperArmSize,
          dist(rightShoulder.x, rightShoulder.y, rightElbow.x, rightElbow.y)
        );
        pop();
        ...
        cylinder(
          lowerArmSize,
          dist(rightElbow.x, rightElbow.y, rightWrist.x, rightWrist.y)
        );
        pop();

        let wordsUL = createGraphics(
          upperLegSize * 2,
          dist(
            leftShoulder.x,
            leftShoulder.y,
        ...
        wordsUL.text(displayedText, 0, 20);

        let wordsLL = createGraphics(
          lowerLegSize * 2,
          dist(
            leftShoulder.x,
            leftShoulder.y,
        ...
        wordsLL.text(displayedText, 0, 20);
        ...
        cylinder(
          upperLegSize,
          dist(leftHip.x, leftHip.y, leftKnee.x, leftKnee.y)
        );
        pop();
        ...
        cylinder(
          lowerLegSize,
          dist(leftKnee.x, leftKnee.y, leftAnkle.x, leftAnkle.y)
        );
        pop();
        ...
        cylinder(
          upperLegSize,
          dist(rightHip.x, rightHip.y, rightKnee.x, rightKnee.y)
        );
        pop();
        ...
        cylinder(
          lowerLegSize,
          dist(rightKnee.x, rightKnee.y, rightAnkle.x, rightAnkle.y)
        );
        pop();
      }
    }
...
function gotPoses(results) {
  ...
  if (results.length > 0) {
    ...
    displayGoing = true;
    // console.log(displayGoing);
    if (!recording && transcriber && !finishedRecording) {
      beginRecord();
    }
  } else {
    humanDetected = false;
...
function filterMask(result, targetParts) {
  ...
  let imageData = filteredContext.getImageData(
    0,
    0,
    filteredMask.width,
    filteredMask.height
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (red === 255 && green === 255 && blue === 255) {
      ...
      for (let j = 0; j < targetParts.length; j++) {
        if (
          red === targetParts[j][0] &&
          green === targetParts[j][1] &&
          blue === targetParts[j][2]
        ) {
          found = true;
          break;
        }
...
async function ask() {
  ...
  let prompt =
      "clothing, clothes, garments, apparel, attire, wear, outfit, threads, garments, vesture, garb, vestments, couture, fashion. This is the prompt for the type of clothing i want: " +
    fullTranscript;
  ...
  if (!result.error) {
    if (!successfull) {
      loadImage(result.output[0], function (loadedImg) {
        // log the image url
        // console.log(result.output[0])

        // save the image url to a global variable
        imageUrl = result.output[0];

        // initialize the array of images in local storage
        ...
        //       localStorage.setItem(counter.toString(), imageUrl);
